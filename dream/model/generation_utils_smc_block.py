...
class DreamGenerationConfig(GenerationConfig):
    def __init__(self, **kwargs):
        ...
        self.num_return_sequences: int = kwargs.pop("num_return_sequences", 4)
...
class DreamGenerationMixin:
    ...
    def _sample(
        ...
        log_p = torch.zeros_like(x, dtype=torch.float32, device=x.device)
        # weight initialization in log-space
        log_w = torch.zeros(num_particles, dtype=torch.float32,).to(x.device)
        ...
        for num_block in range(num_blocks):
            ...
            while True:
                ...
                if alg == 'confidence_threshold':
                    ...
                    current_transfer_tokens = (x[:, current_block_start:current_block_end] == mask_token_id).sum(dim=1)
